...
export default function CVManagerPage() {
  ...
  // Link editor state for selected CV
  ...
  // Sync link editor when selected CV changes
  useEffect(() => {
    if (selected) {
    ...
    startTransition(async () => {
      try {
        const cv = await importCV(
          title.trim(),
          rawText.trim(),
          importGithub.trim() || undefined,
          importPortfolio.trim() || undefined,
        );
        setCVs((prev) => [cv, ...prev]);
        setTitle("");
        setRawText("");
  ...
  function handleSaveLinks() {
    ...
    startTransition(async () => {
      try {
        const updated = await updateCVLinks(
          selected.id,
          linkGithub.trim() || null,
          linkPortfolio.trim() || null,
        );
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: editLinks ? '12px' : '0' }}>
                <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                  {selected.portfolio_url ? (
                    <a href={selected.portfolio_url} target="_blank" rel="noopener noreferrer" style={{ fontFamily: F.body, fontSize: '12px', color: '#FF5722', textDecoration: 'none' }}>
                      ↗ Portfolio
                    </a>
                  ) : (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  <div>
                    ...
                    <input
                      className="forge-input"
                      style={inputStyle}
                      value={importPortfolio}
                      onChange={(e) => setImportPortfolio(e.target.value)}
                      placeholder="https://yoursite.com"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      style={inputStyle}
                      value={importGithub}
                      onChange={(e) => setImportGithub(e.target.value)}
                      placeholder="https://github.com/username"
                    />
                  </div>
                </div>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: importDisabled
                      ? '#1E1E20'
                      : importBtnHovered
                        ? 'linear-gradient(135deg, #FF8C42, #FFC947)'
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '2px' }}>
      <button
        ...
        style={{
          ...
          color: isSelected ? '#E2E2E4' : '#9A9AA4',
          cursor: 'pointer',
          overflow: 'hidden',
      ...
      <button
        ...
        style={{
          ...
          color: delHovered ? '#F87171' : '#5C5C66',
          fontSize: '17px',
          lineHeight: 1,
          flexShrink: 0,
